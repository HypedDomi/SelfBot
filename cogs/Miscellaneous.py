...
class Miscellaneous(commands.Cog):
    ...
    async def snipe(self, ctx, mode=None):
        ...
        if mode == "edit":
            ...
            msg = f"**{message.author}**\n {message.content}"
            await ctx.message.reply(msg.replace("\n", "\n> ") if msg.content, allowed_mentions=discord.AllowedMentions(everyone=False, users=False, roles=False, replied_user=False), files=[await x.to_file() for x in message.attachments])
